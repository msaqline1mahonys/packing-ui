...
const COMMODITY_TYPES_ENDPOINT = `${API_BASE_URL}/reference-data/commodity-types`;
...
function buildDraft(row) {
  const next = { testRequired: "No" };
  for (const field of config.formFields) {
    next[field.key] = row?.[field.key] ?? (field.key === "testRequired" ? "No" : "");
  }
  return next;
}
...
export default function CommodityTypePage() {
  ...
  const loadCommodityTypes = useCallback(async () => {
    ...
    setError("");
    try {
      const result = await commodityTypeRequest("?per_page=500");
      const pager = result?.data;
      const apiRows = Array.isArray(pager?.data) ? pager.data : Array.isArray(pager) ? pager : [];
      setRows(apiRows.map(fromApiCommodityType).filter(Boolean));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to load commodity types.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  async function saveModal() {
    ...
    if (!tenant.organization_id || !tenant.site_id) {
      setError("Organization and current site are required to save a commodity type.");
      return;
    }
    ...
    try {
      ...
      if (modalMode === "add") {
        const result = await commodityTypeRequest("", {
          method: "POST",
          body: JSON.stringify(body),
        });
        const nextRow = fromApiCommodityType(result.data);
        if (!nextRow) throw new Error("Invalid response from server.");
        setRows((prev) => [nextRow, ...prev]);
        setSelectedId(nextRow.id);
        setNotice(result.message || "Commodity type created successfully.");
        setModalMode(null);
        return;
      }

      if (modalMode === "edit" && selected) {
        const result = await commodityTypeRequest(`/${selected.id}`, {
          method: "PUT",
          body: JSON.stringify(body),
        });
        const nextRow = fromApiCommodityType(result.data);
        ...
        setNotice(result.message || "Commodity type updated successfully.");
        setModalMode(null);
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to save commodity type.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function removeSelected() {
    ...
    const shouldDelete = window.confirm(
      `Delete commodity type "${selected.name || selected.acosCode || selected.id}"?`
    );
    ...
    try {
      const result = await commodityTypeRequest(`/${selected.id}`, { method: "DELETE" });
      setRows((prev) => prev.filter((row) => row.id !== selected.id));
      setSelectedId(null);
      setNotice(result.message || "Commodity type deleted successfully.");
    } catch (err) {
  ...
  const toolbarActions = (
    <div className="flex flex-wrap gap-2">
      ...
      <Button type="button" variant="outline" size="sm" onClick={loadCommodityTypes} disabled={isLoading}>
        Refresh
      </Button>
  ...
  return (
    <div className="space-y-5">
      ...
      <Modal
        open={modalMode != null}
        title={modalMode === "edit" ? `Edit ${config.title}` : `Add ${config.title}`}
        onClose={closeModal}
      >
        {modalError ? (
