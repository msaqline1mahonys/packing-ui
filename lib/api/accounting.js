...
export async function savePackBreakdown(packId, payload) {
  return accountingRequest(`/accounting/pack-breakdown/${encodeURIComponent(packId)}`, {
    method: "PUT",
    body: JSON.stringify({ ...getTenantPayload(), ...payload }),
  });
...
export async function bulkCreateInvoices(payload) {
  return accountingRequest("/accounting/invoices/bulk", {
    method: "POST",
    body: JSON.stringify({ ...getTenantPayload(), ...payload }),
  });
}
