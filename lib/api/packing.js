...
import { normalizePackSourceTags } from "@/lib/pack-source-tags";
...
function normalizeContainer(raw) {
  ...
  return {
    ...
    onSite: Boolean(raw.on_site ?? raw.onSite),
    replacesContainerId: raw.replaces_container_id ?? raw.replacesContainerId ?? null,
    replacedByContainerId: raw.replaced_by_container_id ?? raw.replacedByContainerId ?? null,
    replacesContainerNumber:
      raw.replaces_container?.container_number ??
      raw.replacesContainer?.containerNumber ??
      raw.replaces_container_number ??
      raw.replacesContainerNumber ??
      "",
    replacedByContainerNumber:
      raw.replaced_by_container?.container_number ??
      raw.replacedByContainer?.containerNumber ??
      raw.replaced_by_container_number ??
      raw.replacedByContainerNumber ??
      "",
    // Packing Order
    ...
    containerSize: raw.container_code?.container_size ?? raw.container_code?.containerSize ?? raw.containerSize ?? "",
    ...
    releaseId: raw.release_id ?? raw.releaseId ?? raw.release?.id ?? null,
    ...
    packerEditUnlockReason: raw.packer_edit_unlock_reason ?? raw.packerEditUnlockReason ?? "",
    ...
    ecInspectionRemarkCode: raw.ec_inspection_remark_code ?? raw.ecInspectionRemarkCode ?? "",
    ecInspectionRemark: raw.ec_inspection_remark ?? raw.ecInspectionRemark ?? "",
    grainInspectionRemarkCode: raw.grain_inspection_remark_code ?? raw.grainInspectionRemarkCode ?? "",
    grainInspectionRemark: raw.grain_inspection_remark ?? raw.grainInspectionRemark ?? "",
...
function normalizeContainerListRow(raw) {
  const container = normalizeContainer(raw);
  if (!container) return null;

  const pack = raw.pack ?? {};
  const emptyPark = raw.empty_container_park ?? raw.emptyContainerPark ?? null;
  const transporterRecord = raw.transporter_record ?? raw.transporterRecord ?? null;

  return {
    ...container,
    emptyContainerPark: emptyPark,
    transporterRecord,
    emptyContainerParkName: emptyPark?.name ?? container.releasePark ?? "",
    transporterName: transporterRecord?.name ?? container.transporter ?? "",
    packStatus: pack.status ?? raw.pack_status ?? raw.packStatus ?? "",
    jobReference: pack.job_reference ?? pack.jobReference ?? raw.job_reference ?? "",
    packNumber: pack.pack_number ?? pack.packNumber ?? raw.pack_number ?? raw.packNumber ?? "",
    importExport: pack.import_export ?? pack.importExport ?? raw.import_export ?? "",
    packingStartDate: pack.packing_start_date ?? pack.packingStartDate ?? "",
    containersRequired: pack.containers_required ?? pack.containersRequired ?? null,
    customerName: pack.customer?.name ?? pack.customer_name ?? "",
    commodityName: pack.commodity?.description ?? pack.commodity_description ?? "",
    vessel: pack.vessel_voyage?.vessel?.vessel_name ?? pack.vesselVoyage?.vessel?.vesselName ?? pack.vessel ?? "",
    etd: pack.etd ?? pack.vessel_voyage?.vessel_etd ?? "",
    vesselCutoffDate: pack.vessel_cutoff_date ?? pack.vesselCutoffDate ?? "",
    terminalName: pack.terminal?.name ?? "",
  };
}

function normalizeLinkedReleasePark(raw) {
  const park = raw?.container_park ?? raw?.containerPark ?? null;
  const transporters = Array.isArray(raw?.transporters) ? raw.transporters : [];
  return {
    containerParkId: raw?.container_park_id ?? raw?.containerParkId ?? park?.id ?? "",
    containerParkName: park?.name ?? "",
    transporterIds: transporters.map((t) => t?.id ?? t).filter(Boolean),
    transporters: transporters.map((t) => ({ id: t?.id ?? t, name: t?.name ?? "" })),
  };
}
...
function normalizeRelease(raw) {
  ...
  const parks = Array.isArray(nested?.parks)
    ? nested.parks.map(normalizeLinkedReleasePark)
    : Array.isArray(raw.parks)
      ? raw.parks.map((p) =>
          typeof p.containerParkId !== "undefined" || p.container_park_id
            ? {
                containerParkId: p.container_park_id ?? p.containerParkId ?? p.container_park?.id ?? "",
                containerParkName: p.container_park?.name ?? p.containerParkName ?? "",
                transporterIds: Array.isArray(p.transporterIds)
                  ? p.transporterIds
                  : Array.isArray(p.transporters)
                    ? p.transporters.map((t) => t?.id ?? t).filter(Boolean)
                    : [],
                transporters: Array.isArray(p.transporters)
                  ? p.transporters.map((t) => ({ id: t?.id ?? t, name: t?.name ?? "" }))
                  : [],
              }
            : normalizeLinkedReleasePark(p)
        )
      : [];
  ...
  return {
    ...
    containerCodeIsoCode:
      nested?.container_iso_code ?? nested?.containerIsoCode ?? raw.containerCodeIsoCode ?? "",
    parks,
    usage: raw.usage ?? nested?.usage ?? null,
    emptyContainerParkId: raw.empty_container_park_id ?? raw.emptyContainerParkId ?? null,
    transporterId: raw.transporter_id ?? raw.transporterId ?? null,
...
function normalizeSample(raw) {
  ...
  return {
    ...
    requestedDate: stripToDate(raw.requested_date ?? raw.requestedDate ?? createdAt),
    status: raw.status ?? "Pending",
    notes: raw.notes ?? "",
...
function normalizePack(raw) {
  ...
  const releases = Array.isArray(raw.releases)
    ? raw.releases.map(normalizeRelease).filter(Boolean)
    : raw.releases && typeof raw.releases === "object"
      ? Object.values(raw.releases).map(normalizeRelease).filter(Boolean)
      : [];
  ...
  const fumigationDetailRaw = raw.fumigation_detail ?? raw.fumigationDetail ?? null;
  const fumigationDetail = normalizeFumigationDetail(fumigationDetailRaw);
  const fumigants = typeof window !== "undefined" ? loadFumigants() : [];
  const fumigationDisplay = resolvePackFumigantDisplay(
    {
      fumigantId: raw.fumigant_id ?? raw.fumigantId,
      fumigationRequired: raw.fumigation_required ?? raw.fumigationRequired,
      fumigationTiming: raw.fumigation_timing ?? raw.fumigationTiming,
      fumigation: raw.fumigation,
      fumigationDetail,
    },
    fumigants
  );
  ...
  const normalized = {
    ...
    packNumber: raw.pack_number ?? raw.packNumber ?? "",
    date: raw.date,
    ...
    packingLocationId: raw.packing_location_id ?? raw.packingLocationId ?? null,
    packingLocation: raw.packing_location ?? raw.packingLocation ?? null,
    packingLocationName:
      raw.packing_location?.name ??
      raw.packingLocation?.name ??
      raw.packing_location_name ??
      raw.packingLocationName ??
      "",
    ...
    transportInvoice: raw.transport_invoice ?? raw.transportInvoice ?? "",
    ...
    vesselScheduleUpdatedAt: raw.vessel_schedule_updated_at ?? raw.vesselScheduleUpdatedAt ?? null,
    vesselScheduleAcknowledgedAt: raw.vessel_schedule_acknowledged_at ?? raw.vesselScheduleAcknowledgedAt ?? null,
    fumigationTiming: raw.fumigation_timing ?? raw.fumigationTiming ?? "",
    ...
    containerSourceTags: normalizePackSourceTags(raw.container_source_tags ?? raw.containerSourceTags),
    ...
    releases: releases,
    linkedReleaseIds: releases.map((r) => r.releaseId).filter(Boolean),
    containers,
    ...
    // Blend pack
    isBlend: Boolean(raw.is_blend ?? raw.isBlend),
    blendComponents: (() => {
      const components = raw.blend_components ?? raw.blendComponents;
    ...
    blendTransferId: raw.blend_transfer_id ?? raw.blendTransferId ?? null,
    blendPerformedAt: raw.blend_performed_at ?? raw.blendPerformedAt ?? null,
  };
  normalized.blendPending = Boolean(normalized.isBlend && !normalized.blendPerformedAt);
  return normalized;
}

export async function performBlend(packId, components) {
  const raw = await packingRequest(`/packing/packs/${packId}/blend`, {
    method: "POST",
    body: JSON.stringify({ ...getTenantPayload(), components }),
  });
...
export async function listPacks(params = {}) {
  ...
  const rows = items.map(normalizePack).filter(Boolean);
  const pagination = data && !Array.isArray(data) ? {
    currentPage: data.current_page ?? 1,
    lastPage: data.last_page ?? 1,
    perPage: data.per_page ?? rows.length,
...
export async function listContainers(params = {}) {
  const { packStatus, importExport, dateField, from, to, on, search, page, perPage } = params;
  const qs = new URLSearchParams();
  if (Array.isArray(packStatus) && packStatus.length) {
    packStatus.forEach((s) => qs.append("pack_status[]", s));
  } else if (typeof packStatus === "string" && packStatus) {
    qs.append("pack_status[]", packStatus);
  }
  if (importExport && importExport !== "all") qs.set("import_export", importExport);
  if (dateField) qs.set("date_field", dateField);
  if (from) qs.set("from", from);
  ...
  if (perPage) qs.set("per_page", String(perPage));
  const query = qs.toString();
  const data = await packingRequest(`/packing/containers${query ? `?${query}` : ""}`);
  const items = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : [];
  const rawRows = Array.isArray(items) ? items : [];
  const rows = rawRows.map(normalizeContainerListRow).filter(Boolean);
  const pagination = data && !Array.isArray(data) ? {
    currentPage: data.current_page ?? 1,
    lastPage: data.last_page ?? 1,
    perPage: data.per_page ?? rows.length,
...
function normalizeDuplicateMatches(rawMatches) {
  return (Array.isArray(rawMatches) ? rawMatches : []).map((row) => ({
    ...
    packStatus: row.pack_status ?? row.packStatus ?? "",
    packingStartDate: row.packing_start_date ?? row.packingStartDate ?? "",
    packDate: row.pack_date ?? row.packDate ?? "",
    customerName: row.customer_name ?? row.customerName ?? "",
  }));
}

export async function checkContainerDuplicate({
  containerNumber,
  excludePackId,
  excludeContainerId,
  months = 3,
} = {}) {
  const qs = buildTenantQuery({
    container_number: containerNumber,
    exclude_pack_id: excludePackId,
    exclude_container_id: excludeContainerId,
    months,
  });
  const raw = await packingRequest(`/packing/containers/duplicate-check${qs}`);
  return {
    containerNumber: raw.container_number ?? raw.containerNumber ?? containerNumber,
    months: raw.months ?? months,
    matches: normalizeDuplicateMatches(raw.matches),
  };
}

export async function checkSealDuplicate({
  sealNumber,
  excludePackId,
  excludeContainerId,
  months = 3,
  ...
  const raw = await packingRequest(`/packing/containers/seal-duplicate-check${qs}`);
  return {
    sealNumber: raw.seal_number ?? raw.sealNumber ?? sealNumber,
    months: raw.months ?? months,
    matches: normalizeDuplicateMatches(raw.matches),
  };
}

function normalizeJobReferenceDuplicateMatches(rawMatches) {
  return (Array.isArray(rawMatches) ? rawMatches : []).map((row) => ({
    packId: row.pack_id ?? row.packId,
    jobReference: row.job_reference ?? row.jobReference ?? "",
    packNumber: row.pack_number ?? row.packNumber ?? "",
...
export async function checkJobReferenceDuplicate({
  jobReference,
  excludePackId,
} = {}) {
  const qs = buildTenantQuery({
  ...
  const raw = await packingRequest(`/packing/packs/job-reference-duplicate-check${qs}`);
  return {
    jobReference: raw.job_reference ?? raw.jobReference ?? jobReference,
    matches: normalizeJobReferenceDuplicateMatches(raw.matches),
  };
}
...
export async function createPack(payload) {
  const { packNumber, pack_number, ...rest } = payload ?? {};
  return packingRequest("/packing/packs", {
    method: "POST",
    body: JSON.stringify({ ...getTenantPayload(), ...rest }),
  });
}
...
export async function acknowledgeVesselScheduleUpdate(packId) {
  const raw = await packingRequest(`/packing/packs/${packId}/acknowledge-vessel-schedule`, {
    method: "PATCH",
...
export async function createContainerReplacement(packId, containerId) {
  const raw = await packingRequest(`/packing/packs/${packId}/containers/${containerId}/replacement`, {
    method: "POST",
    body: JSON.stringify(getTenantPayload()),
  });
  return normalizePack(raw);
...
export async function removePackContainers(packId, containerIds) {
  const raw = await packingRequest(`/packing/packs/${packId}/containers/remove`, {
    method: "POST",
    body: JSON.stringify({
      ...getTenantPayload(),
