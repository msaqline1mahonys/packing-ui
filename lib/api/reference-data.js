...
export async function fetchWeighbridgesList() {
  return referenceDataGet("/reference-data/weighbridges?per_page=500", "Unable to load weighbridges.");
}

/** Pull a live weight (in tons) from a weighbridge via its Power Automate connector. */
export async function readWeighbridge(id, body = {}) {
  const response = await fetch(`${API_BASE}/reference-data/weighbridges/${id}/read`, {
    method: "POST",
    headers: getAuthHeaders(),
  ...
  const result = await response.json().catch(() => null);
  if (!response.ok || result?.success === false) {
    throw new Error(extractApiError(result, "Unable to read weight from the weighbridge."));
  }
  return result?.data ?? result;
}
...
export async function fetchVesselVoyagesList({ search = "", perPage = 500 } = {}) {
  const params = new URLSearchParams({ per_page: String(perPage) });
  const trimmedSearch = String(search || "").trim();
  if (trimmedSearch) params.set("search", trimmedSearch);
  return referenceDataGet(`/reference-data/vessel-voyages?${params}`, "Unable to load vessel voyages.");
}

export async function fetchVesselVoyageById(id) {
  if (!id) return null;
  return referenceDataRequest(`/reference-data/vessel-voyages/${id}`, "Unable to load vessel voyage.");
}
