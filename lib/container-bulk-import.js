...
import {
  bulkImportStatusForPark,
  isContainerSlotEligibleForBulkImport,
  isContainerStatusProtected,
  isMatchedStatus,
...
function rankEmptySlots(
  ...
  const empty = sortByOrder(containers).filter((container) =>
    isContainerSlotEligibleForBulkImport(container, fieldName)
  );
...
export function planBulkImport({
  ...
  const rows = numbers.map((number, index) => {
    ...
    if (proposedSlot && isContainerStatusProtected(proposedSlot)) {
      conflictType = conflictType || "matched_protected";
      defaultAction = "skip";
    }

    if (existingSlot && isMatchedStatus(existingSlot.status)) {
      conflictType = conflictType || "matched_protected";
      defaultAction = "skip";
    }
    ...
    if (blocked || conflictType === "matched_protected") action = "skip";
...
export function applyBulkImport({
  ...
  const parkId =
    selectedRelease?.emptyContainerParkId ?? selectedRelease?.empty_container_park_id ?? null;
  const importStatus = bulkImportStatusForPark(parkId, {
    ...lookupOptions,
    releasePark: releasePatch.releasePark,
  });
  ...
  planRows.forEach((row) => {
    ...
    const target = containers.find((container) => container.id === row.targetSlotId);
    if (target && isContainerStatusProtected(target)) return;

    const patch = { ...releasePatch, status: importStatus };
