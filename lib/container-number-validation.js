...
/** 6–8 alphanumeric characters; hyphens allowed between characters. */
export const SEAL_NUMBER_REGEX = /^[A-Z0-9](?:[A-Z0-9-]{4,6}[A-Z0-9])?$/;

const CONTAINER_NUMBER_MAX_LENGTH = 11;
export const SEAL_NUMBER_MIN_LENGTH = 6;
export const SEAL_NUMBER_MAX_LENGTH = 8;
...
export function validateSealNumber(value) {
  ...
  if (
    normalized.length < SEAL_NUMBER_MIN_LENGTH ||
    normalized.length > SEAL_NUMBER_MAX_LENGTH
  ) {
    return `Seal number must be ${SEAL_NUMBER_MIN_LENGTH} to ${SEAL_NUMBER_MAX_LENGTH} characters.`;
  }

  if (!SEAL_NUMBER_REGEX.test(normalized)) {
    return `Seal number must be ${SEAL_NUMBER_MIN_LENGTH} to ${SEAL_NUMBER_MAX_LENGTH} characters and use only letters, digits, and hyphens (e.g. SL1234 or SL-123).`;
  }
