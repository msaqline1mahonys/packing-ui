...
export function resolveSyncedContainerStatus(sourceContainer, existing) {
  ...
  if (serverStatus === CONTAINER_STATUS.MATCHED) return CONTAINER_STATUS.MATCHED;
  if (serverStatus && serverStatus !== CONTAINER_STATUS.PENDING) return serverStatus;
  if (localStatus === CONTAINER_STATUS.MATCHED) return CONTAINER_STATUS.MATCHED;
  return serverStatus || localStatus || CONTAINER_STATUS.PENDING;
}
