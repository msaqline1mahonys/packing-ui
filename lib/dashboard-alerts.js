...
function buildContainerAlerts(containers, packsById, today) {
  ...
  for (const c of containers) {
    ...
    const stage = displayContainerStage(c, isImport, containerStage);
    ...
    const onSite = Boolean(c.onSite ?? c.on_site);
    if (packStatus === "Inprogress" && containerNum && stage === "Available to Pack" && !onSite) {
      alerts.push({
