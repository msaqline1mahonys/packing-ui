...
import { CONTAINER_STAGE_OPTIONS } from "@/lib/packing-container-ui";
...
import {
  averageMinutes,
  buildTurnaroundExportRows,
  buildTurnaroundRow,
  defaultTurnaroundDateRange,
  ticketDateInRange,
} from "@/lib/ticket-turnaround";

const STAGE_KEYS = [...CONTAINER_STAGE_OPTIONS];
...
function isCancelledTicket(ticket) {
  return String(ticket.status ?? "").toLowerCase() === "cancelled";
}

function turnaroundMinutesValue(ticket) {
  const n = Number(ticket.truckTurnaroundMinutes ?? ticket.truck_turnaround_minutes);
  return Number.isFinite(n) ? n : null;
}

function buildTurnaroundMetrics(tickets, { today, dateFrom = null, dateTo = null } = {}) {
  const completedAll = tickets.filter(
    (ticket) => isCompletedTicket(ticket) && !isCancelledTicket(ticket),
  );
  const completedInRange = completedAll.filter((ticket) => {
    if (!dateFrom && !dateTo) return true;
    return ticketDateInRange(ticket, dateFrom, dateTo);
  });

  const completedToday = completedAll
    .filter((ticket) => isTodayTicket(ticket, today))
    .map((ticket) => buildTurnaroundRow(ticket))
    .filter((row) => row.turnaroundMinutes != null)
    .sort((a, b) => b.turnaroundMinutes - a.turnaroundMinutes);

  const completedInRangeRows = completedInRange
    .map((ticket) => buildTurnaroundRow(ticket))
    .filter((row) => row.turnaroundMinutes != null)
    .sort((a, b) => b.turnaroundMinutes - a.turnaroundMinutes);

  const onSiteNow = tickets
    .filter((ticket) => !isCompletedTicket(ticket) && !isCancelledTicket(ticket))
    .map((ticket) => buildTurnaroundRow(ticket, { includeOnSite: true }))
    .filter((row) => row.turnaroundMinutes != null)
    .sort((a, b) => b.turnaroundMinutes - a.turnaroundMinutes);

  const exportRows = buildTurnaroundExportRows(
    completedInRange.filter((ticket) => turnaroundMinutesValue(ticket) != null),
  );

  return {
    avgMinutesToday: averageMinutes(completedToday.map((row) => row.turnaroundMinutes)),
    avgMinutesInRange: averageMinutes(
      completedInRangeRows.map((row) => row.turnaroundMinutes),
    ),
    trucksOnSiteCount: onSiteNow.length,
    avgOnSiteMinutes: averageMinutes(onSiteNow.map((row) => row.turnaroundMinutes)),
    completedToday,
    completedInRange: completedInRangeRows,
    onSiteNow,
    exportRows,
    dateFrom,
    dateTo,
...
export function buildDashboardMetrics({
  ...
  for (const container of containers) {
    ...
    const stage = displayContainerStage(container, isImport, containerStage);
    if (containersByStage[stage] != null) containersByStage[stage] += 1;
    else containersByStage["Available to Pack"] += 1;
  ...
  const turnaround = buildTurnaroundMetrics(tickets, {
    today,
    ...defaultTurnaroundDateRange(),
  });
