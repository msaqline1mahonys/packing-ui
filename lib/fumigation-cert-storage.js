// Fumigation Certificate — sessionStorage snapshot (per-session draft state)
// and localStorage audit trail (issued copies).

const SNAP_PREFIX = "fumigation-cert:";
const ISSUES_KEY = "packing-ui-fumigation-certificate-issues";

// ─── sessionStorage helpers (draft while editing) ────────────────────────────

export function saveFumigationCertSnapshot(packId, snapshot) {
...
export function loadFumigationCertSnapshot(packId) {
...
export function clearFumigationCertSnapshot(packId) {
  if (typeof window === "undefined" || packId == null) return;
...
export function saveCertificateIssue(packId, certModel) {
  ...
  issues.unshift({ packId, issuedAt, issuedBy, payload: certModel });
  writeIssues(issues);
  return issuedAt;
}

/** Returns all issues for this pack, newest first. */
export function loadCertificateIssues(packId) {
  return readIssues().filter((i) => i.packId === packId);
}

/** Returns the payload of a specific issued copy, or null. */
export function loadCertificateIssue(packId, issuedAt) {
