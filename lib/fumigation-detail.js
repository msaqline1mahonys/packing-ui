...
const SNAKE_TO_CAMEL = {
  ...
  concentration_readings: "concentrationReadings",
...
/** Section D fields stored on pack fumigation_detail (concentration readings + monitoring). */
export function sectionDFieldsFromRecord(record) {
  ...
  return {
    monitoringDeviceSerials: record.monitoringDeviceSerials ?? "",
    dosingFinishAt: record.dosingFinishAt ?? "",
    ventilationStartAt: record.ventilationStartAt ?? "",
    concentrationReadings: Array.isArray(record.concentrationReadings) ? record.concentrationReadings : [],
    topUpEntries: Array.isArray(record.topUpEntries) ? record.topUpEntries : [],
  };
}

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/** True when value looks like a backend UUID (not a human-readable label). */
...
function pickDisplayLabel(...candidates) {
  for (const candidate of candidates) {
    ...
    if (!label || label === "—" || label === "Required" || looksLikeUuid(label)) continue;
    return label;
  }
  return "";
}

/** Fumigant block for certificate/record documents — never falls back to a raw UUID. */
export function resolveFumigantDisplayModel(fumigant, fumigantId, pack, fd = {}) {
  if (fumigant?.name) {
    return {
      name: fumigant.name,
      code: fumigant.code ?? "",
  ...
  const name =
    pickDisplayLabel(
      ...
      fd.fumigantName,
      pack?.fumigation,
      pack?.fumigationTiming ?? pack?.fumigation_timing,
      looksLikeUuid(fumigantId) ? "" : fumigantId,
    ) || "Fumigation";

  return { name, code: "", activeConstituent: "", productForm: "" };
}

/** Methodology display name — never falls back to a raw UUID. */
export function resolveMethodologyDisplayName(methodology, methodologyId, pack) {
  if (methodology?.name) return methodology.name;
  const nested = pack?.methodology;
  const nestedName =
    nested && typeof nested === "object"
      ? String(nested.name ?? "").trim()
      : typeof nested === "string"
        ...
        : "";
  return (
    pickDisplayLabel(nestedName, looksLikeUuid(methodologyId) ? "" : methodologyId) ||
