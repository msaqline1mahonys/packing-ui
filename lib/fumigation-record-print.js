...
/** Resolve a record using templates loaded from the API (logos, sections, footer). */
export async function resolveFumigationRecordAsync(packId, packOverride = null) {
  const { fetchRecordTemplatesNormalized } = await import("@/lib/api/fumigation");
  const recordTemplates = await fetchRecordTemplatesNormalized();
  return resolveFumigationRecord(packId, packOverride, { recordTemplates });
}
