...
import { resolveEarlyContainerStatus } from "@/lib/container-status";
...
export function buildContainerApiRecord(container, packRow = {}) {
  return {
    ...
    status: (() => {
      ...
      const early = resolveEarlyContainerStatus(container);
      const earlyStages = ["Pending", "Matched", "Available to Pack"];
      return earlyStages.includes(stage) ? early : stage;
    })(),
  };
}
