...
export function getPackSamples(row) {
  ...
  return raw
    .filter(Boolean)
    .filter((sample) => String(sample?.status ?? "").toLowerCase() !== "cancelled");
}
...
export async function cancelPackSample(sampleId) {
  return updatePackSample(sampleId, { status: "Cancelled" });
}
