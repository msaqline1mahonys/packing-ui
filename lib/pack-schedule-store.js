...
const PACK_ROW_TEMPLATE_BY_ID = new Map(PACK_SCHEDULE_ROWS.map((row) => [Number(row.id), row]));
...
function mergePackAttachmentFilesFromSeed(row) {
  const template = PACK_ROW_TEMPLATE_BY_ID.get(Number(row.id));
  if (!template) return row;
  const packId = Number(row.id);
  let next = row;
  let changed = false;
  for (const key of ATTACHMENT_FILE_KEYS) {
    ...
    /** Same file count as seed but URLs/names in Data.js changed — replace stale localStorage demo attachments. */
