...
export function isEmptyEcReplacementDraft(container) {
  return containerStage(container) === "Draft" && !containerHasEnteredPackingDetails(container);
}
...
function emptyInspectionStatus(container) {
  return String(container?.emptyInspection ?? container?.empty_inspection ?? "Pending");
}
...
export function canToggleOnSite(stage) {
  return stage === "Draft" || stage === "In Progress" || stage === "ECI Approved";
}

export function containerStage(container, isImport = false) {
  if (isImport) {
    if (!getContainerNumberFromRecord(container)) return "Draft";
    return containerHasCompleteChecks(container, { isImport: true }) ? "Completed" : "In Progress";
  }

  if (isEcFailedContainer(container)) return "ECI Failed";
  ...
  if (isAccepted && checksComplete) return "Completed";
  if (isAccepted) return "PRA Passed";
  if (isFailed) return "PRA Failed";
  if (container?.praSubmitted) return "PRA Submitted";

  if (emptyInspectionStatus(container) === "Passed") return "ECI Approved";

  if (getContainerNumberFromRecord(container)) return "In Progress";
