...
import { CONTAINER_STATUS, displayContainerStage, isEffectivelyMatchedContainer, normalizeContainerStoredStatus, resolveSyncedContainerStatus } from "@/lib/container-status";
...
function newContainerDraft({ packRow, index, sourceContainer, parkName, transporterName, firstPackRelease, stockLocations }) {
  ...
  const draft = {
    ...
    status: normalizeContainerStoredStatus(sourceContainer?.status) || CONTAINER_STATUS.PENDING,
        ...
        applyDraftContainerPrefills(
          {
            ...
            order: index + 1,
            id: resolvedId,
            status: resolveSyncedContainerStatus(sourceContainer, existing),
            ...
            emptyInspection:
              String(sourceContainer?.emptyInspection ?? sourceContainer?.empty_inspection ?? "").trim() ||
              existing.emptyInspection ||
              "Pending",
            grainInspection:
              String(sourceContainer?.grainInspection ?? sourceContainer?.grain_inspection ?? "").trim() ||
              existing.grainInspection ||
              "Pending",
            aoInspectionRemark:
...
/** True when a packer has entered meaningful packing data (not planner/logistics or UI prefills). */
...
export function isEmptyEcReplacementDraft(container) {
  return containerStage(container) === CONTAINER_STATUS.PENDING && !containerHasEnteredPackingDetails(container);
}
...
export function containerIsOnSite(container) {
  return Boolean(container?.onSite ?? container?.on_site);
}

/** Stages where on_site can still be toggled from the containers grid/sidebar. */
export function canToggleOnSite(stage) {
  return (
    stage === CONTAINER_STATUS.PENDING ||
    stage === CONTAINER_STATUS.MATCHED ||
    stage === CONTAINER_STATUS.AVAILABLE_TO_PACK
  );
}

export function containerStage(container, isImport = false) {
  if (isImport) {
    if (!getContainerNumberFromRecord(container)) return CONTAINER_STATUS.PENDING;
    return containerHasCompleteChecks(container, { isImport: true })
      ? CONTAINER_STATUS.COMPLETED
      : CONTAINER_STATUS.AVAILABLE_TO_PACK;
  }

  if (isEcFailedContainer(container)) return CONTAINER_STATUS.ECI_FAILED;
  ...
  if (isAccepted && checksComplete) return CONTAINER_STATUS.COMPLETED;
  if (isAccepted) return CONTAINER_STATUS.PRA_PASSED;
  if (isFailed) return CONTAINER_STATUS.PRA_FAILED;
  if (container?.praSubmitted) return CONTAINER_STATUS.PRA_SUBMITTED;

  const packStatus = String(container?.packStatus ?? container?.pack_status ?? "");
  if (packStatus === CONTAINER_STATUS.AWAITING_APPROVAL && containerHasEnteredPackingDetails(container)) {
    return CONTAINER_STATUS.AWAITING_APPROVAL;
  }

  if (isEffectivelyMatchedContainer(container)) return CONTAINER_STATUS.MATCHED;

  if (getContainerNumberFromRecord(container)) return CONTAINER_STATUS.AVAILABLE_TO_PACK;

  return CONTAINER_STATUS.PENDING;
}

export function countOnSiteContainers(containers) {
  if (!Array.isArray(containers)) return 0;
  return containers.filter((c) => containerIsOnSite(c)).length;
}

export function countAvailableToPackContainers(containers, isImport = false) {
  if (!Array.isArray(containers)) return 0;
  return containers.filter(
    (c) => displayContainerStage(c, isImport, containerStage) === CONTAINER_STATUS.AVAILABLE_TO_PACK,
  ).length;
}

export function countEcFailedContainers(containers, isImport = false) {
  if (!Array.isArray(containers)) return 0;
  return containers.filter((c) => containerStage(c, isImport) === "ECI Failed").length;
}
