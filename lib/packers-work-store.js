...
function toContainerDrafts(packRow, previousPackDraft, parkNames, transporterNames, defaultPacker, defaultAo) {
  ...
  for (let index = 0; index < requiredCount; index += 1) {
    ...
    const parkName = parkNames.get(Number(sourceContainer.emptyContainerParkId)) || "";
    const transporterName = transporterNames.get(Number(sourceContainer.transporterId)) || "";
...
export function syncWorkDrafts(packRows, previousWork = {}) {
  ...
  for (const row of packRows) {
    ...
    next[row.id] = {
      ...
      pemsSubmissions: Array.isArray(previousPackDraft?.pemsSubmissions) ? previousPackDraft.pemsSubmissions : [],
      packChecks: {
        ...DEFAULT_PACK_CHECKS,
