import { canToggleSiteStage, containerStage } from "@/lib/packers-work-store";
...
export function displayContainerStage(row) {
  ...
  const stage = containerStage(row, isImport);
...
export function decoratePackScopedContainerRow(container, {
  ...
  const containerNumber =
    String(container.containerNumber ?? container.containerNo ?? "").trim() ||
    (Number(container.order) > 0 ? `#${container.order}` : "");
 ...
 * @param {{ onToggleSiteStage?: (row: object) => void, savingSiteStageId?: string|null }} options
 */
export function buildContainerGridColumns({
  onToggleSiteStage = null,
  savingSiteStageId = null,
} = {}) {
    ...
    if (column.key === "stage") {
      return {
        ...
        renderCell: ({ row, formattedValue }) => {
          ...
          const toggleable = onToggleSiteStage && canToggleSiteStage(stage);
          if (!stage) return "";
