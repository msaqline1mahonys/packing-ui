...
export function loadPacksReadyToInvoice() {
  return loadPackScheduleRows()
    .filter((row) => row.status === READY_TO_INVOICE_STATUS)
    .map(scheduleRowToInvoicePack);
}

export function findPackReadyToInvoice(packId) {
  return loadPacksReadyToInvoice().find((pack) => pack.id === String(packId)) || null;
}
