...
export function buildPemsStagingSnapshotHtml(submission) {
  ...
  const additionalDeclaration = submission?.rfpAdditionalDeclarationRequired ? "Yes" : "No";
  ...
  const headerFields = isGppir
    ? [
        ...
        fieldBox("Exporter", submission?.exporter),
        fieldBox("Total Quantity", `${totalWeight.toFixed(4)} M/TONS`),
        fieldBox("Estimated Net Metric Weight and Unit", `${totalWeight.toFixed(2)} TONS`),
  ...
  const footerFields = isGppir
    ? [
        ...
        fieldBox("Total Passed", `${passedWeight.toFixed(4)} M/TONS`),
        fieldBox("Total Failed", `${failedWeight.toFixed(4)} M/TONS`),
      ].join("")
    : [
        fieldBox("Submitted AO Name", submission?.aoSignoff),
        fieldBox("Submitted AO Number", submission?.aoNumber),
  ...
  const tableHead = isGppir
    ? "<th>RFP Line No</th><th>Container Number</th><th>Source</th><th>Commodity</th><th>Package Number</th><th>Type</th><th>Weight</th><th>Unit</th><th>Sampled</th><th>Result</th><th>Inspection AO Name</th><th>Remarks</th>"
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .footer-fields { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 8px; margin-top: 12px; }
