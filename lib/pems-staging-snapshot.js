  ...
  return rows
    .map(
      (row) => {
        ...
        <td>${escapeHtml(getContainerInspectionRemark(row, "goods") || "N/A")}</td>
      </tr>`;
      }
    )
    .join("");
  ...
  return rows
    .map(
      (row) => `<tr>
        ...
        <td>${escapeHtml(submission?.aoSignoff || "")}</td>
        <td>${escapeHtml(getContainerInspectionRemark(row, "ec") || "N/A")}</td>
      </tr>`
    )
    .join("");
}
...
export function buildPemsStagingSnapshotHtml(submission) {
  ...
  const tableHead = isGppir
    ? "<th class=\"col-compact\">RFP Line No</th><th>Container Number</th><th>Source</th><th>Commodity Grade</th><th class=\"col-compact\">Package Number</th><th>Type</th><th>Weight</th><th>Unit</th><th>Line Weight</th><th>Unit</th><th>Sampled</th><th>Result</th><th>Inspection AO Name</th><th>Remarks</th>"
