...
export function computeDehireByAt(availableAt, freeDays) {
...
/** @deprecated Use computeDehireByAt */
export const computeReleaseExpiry = computeDehireByAt;
...
export function normalizeRelease(row) {
  ...
  return {
    ...
    dehireByAt:
      row.dehireByAt ?? computeDehireByAt(row.releaseAvailableAt, row.freeDays),
    containerCount: row.containerCount ?? "",
    containerCodeIsoCode: row.containerCodeIsoCode ?? "",
    releaseExpiryAt: row.releaseExpiryAt ?? "",
