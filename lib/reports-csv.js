...
const TRANSACTION_COLUMNS = [
  ...
  { key: "ticketId", label: "Ticket", get: (r) => (r.ticketId ?? r.ticket_id ? `#${String(r.ticketId ?? r.ticket_id).slice(0, 8)}` : "") },
...
const CONTAINER_COLUMNS = [
  ...
  { key: "containerCode", label: "Size", get: (r) => {
    const size = r.containerSize ?? r.container_size;
    if (size) return displayCell(size);
    const rel = r.container_code ?? (typeof r.containerCode === "object" ? r.containerCode : null);
    if (rel && typeof rel === "object") {
      return displayCell(rel.container_size ?? rel.containerSize ?? rel.iso_code ?? rel.isoCode);
    }
    return displayCell(r.containerCode ?? r.container_code);
  } },
  ...
  { key: "commodity", label: "Commodity", get: (r) => displayCell(r.commodity) },
