...
const TICKET_COLUMNS = [
  ...
  { key: "commodityDisplay", label: "Commodity", get: (r) => r.commodityDisplay || r.commodityGrade || "" },
  ...
  { key: "commodityConfirmed", label: "Commodity Confirmed", get: (r) => (r.commodityConfirmed ? "Yes" : "No") },
  { key: "commodityOverrideReason", label: "Commodity Override Reason" },
...
const PACK_COLUMNS = [
  ...
  { key: "commodityDisplay", label: "Commodity", get: (r) => r.commodityDisplay || displayCell(r.commodity) },
...
function buildSummary(report, { source, cadenceLabel, ranBy }) {
  ...
  if (report.customers?.length > 1) {
    ...
    lines.push(`Commodity: ${report.commodity.description || report.commodity.commodityCode || ""}`);
  } else {
