...
function containerReportRow(pack, container, orderFallback) {
  return {
    packId: pack.id,
    customer: packCustomerLabel(pack),
    commodity: packCommodityLabel(pack),
    packDate: pack.date || "",
    vessel: pack.vessel || "",
    jobReference: packJobReference(pack),
    order: container.order ?? orderFallback,
    containerNumber: container.containerNumber ?? container.container_number ?? container.containerNo ?? "",
    containerCode: containerSizeLabel(container, pack),
    isoCode: containerIsoLabel(container),
    sealNumber: containerSealLabel(container),
    releaseNumber: container.releaseNumber ?? container.release_number ?? "",
    status: container.status || "",
    ecrSubmitted: Boolean(container.ecrSubmitted ?? container.ecr_submitted),
  };
}
...
export async function collectReportData({ dateRange, customerId = null, commodityIds = [], sections = ALL_SECTIONS } = {}) {
  ...
  if (includeContainers) {
    ...
    for (const p of visiblePacks) {
      const packContainers = Array.isArray(p.containers) ? p.containers : [];
      packContainers.forEach((c, index) => {
        containers.push(containerReportRow(p, c, index + 1));
      });
    }
  }
