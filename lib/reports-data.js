...
import { COMMODITY_MASTER_ROWS, CUSTOMER_CONTACT_ROWS, CUSTOMER_MASTER_ROWS, PACK_FORM_LOOKUPS } from "@/lib/Data";
...
export function getCustomerDirectory() {
  ...
  // PACK_FORM_LOOKUPS.customers carries names referenced by packs that don't exist in master (e.g. "Riverina Co-op").
  for (const row of PACK_FORM_LOOKUPS.customers || []) {
    if (!byId.has(Number(row.id))) {
      byId.set(Number(row.id), { id: Number(row.id), code: "", name: row.name, emails: [], contacts: [] });
    }
  }
...
export function getCommodityDirectory() {
  ...
  // PACK_FORM_LOOKUPS.commodities references items not in the master (e.g. "Feed barley").
  const seen = new Set(rows.map((r) => Number(r.id)));
  for (const c of PACK_FORM_LOOKUPS.commodities || []) {
    if (!seen.has(Number(c.id))) {
      rows.push({ id: Number(c.id), description: c.description, commodityCode: "", commodityTypeName: "" });
    }
  }
...
function getPacksAll() {
  try {
    return loadPackScheduleRows();
  } catch {
    return [];
  }
}
...
export async function collectReportData({ dateRange, customerId = null, commodityIds = [], sections = ALL_SECTIONS } = {}) {
  ...
  const packsAll = includePacks || includeContainers || includeTickets ? getPacksAll() : [];
