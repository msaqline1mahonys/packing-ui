...
function ticketMatchesCustomer(ticket, customer) {
  if (!customer) return true;
  if (ticket?.accountType === "internal" || ticket?.account_type === "internal") return false;
...
function buildReportLabel({ customer, customers, commodity }) {
  if (commodity) {
    ...
    const commodityName = commodity.description || commodity.commodityCode || "Commodity";
...
async function getContainersAll() {
  try {
    const { rows } = await fetchContainerRows({ perPage: 500 });
    return Array.isArray(rows) ? rows : [];
  } catch {
    return [];
  }
...
function enrichReportPack(pack, customerDirectory, commodityDirectory) {
  ...
  return {
    ...
    onSiteContainers: containers.filter((c) => c.onSite ?? c.on_site).length,
  };
}
...
function containerReportRowFromList(container, pack) {
  ...
  return {
    ...
    onSite: container.onSite ? "Yes" : "No",
...
function containerMatchesReportFilters(container, pack, customers, commodityIds, commodityDirectory, range) {
  if (pack) {
    return (
      dateInRange(pack.date, range) &&
      customerMatchesAny(pack, customers) &&
      commodityMatches(pack, commodityIds, commodityDirectory)
    );
  }
  const packDate = container.packingStartDate ?? container.packDate ?? "";
  return (
    dateInRange(packDate, range) &&
