...
export const DEFAULT_SITE_ROWS = [
  { id: "1", name: "Melbourne", yardNo: "6851", treatmentProviderId: "" },
  { id: "2", name: "Sydney", yardNo: "1002", treatmentProviderId: "" },
  { id: "3", name: "Brisbane", yardNo: "1003", treatmentProviderId: "" },
];
...
export function normalizeSiteRows(rows) {
  ...
  return rows
    .map((row) => {
      ...
      const treatmentProviderId = String(row?.treatmentProviderId ?? "").trim();
      if (!id || !name) return null;
      return { id, name, yardNo, treatmentProviderId };
    })
    .filter(Boolean);
}
