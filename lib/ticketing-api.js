...
import { findDuplicateTickets } from "@/lib/ticket-duplicate-check";
...
function cmoCommodityLineFromApi(line) {
  if (!line) return null;
  return {
    id: line.id,
    commodityTypeId: line.commodity_type_id ?? line.commodityTypeId ?? line.commodity_type?.id ?? line.commodityType?.id ?? "",
    commodityId: line.commodity_id ?? line.commodityId ?? line.commodity?.id ?? "",
    commodityType: line.commodity_type?.name ?? line.commodityType?.name ?? "",
    commodity: line.commodity?.description ?? line.commodity?.commodity_code ?? line.commodity?.name ?? "",
  };
}

export function cmoFromApi(row) {
  if (!row) return null;
  const commodityLines = (row.cmo_commodities ?? row.cmoCommodities ?? [])
    .map(cmoCommodityLineFromApi)
    .filter(Boolean);
  const legacyLine =
    row.commodity_id || row.commodityId
      ...
      : null;
  const commodities = commodityLines.length ? commodityLines : legacyLine ? [legacyLine] : [];
  const first = commodities[0] ?? {};
  const commodityIds = commodities.map((line) => line.commodityId).filter(Boolean);
  ...
  return {
    ...
    commodityTypeId: row.commodity_type_id ?? row.commodityTypeId ?? first.commodityTypeId ?? "",
    ...
    commodityType: row.commodity_type?.name ?? row.commodityType?.name ?? first.commodityType ?? "",
    commodity: commodities.map((line) => line.commodity).filter(Boolean).join(", ") || first.commodity || "",
...
export function cmoToApi(cmo) {
  const commodityIds = Array.isArray(cmo.commodityIds) && cmo.commodityIds.length
    ? cmo.commodityIds
    : Array.isArray(cmo.commodities) && cmo.commodities.length
      ? cmo.commodities.map((line) => line.commodityId).filter(Boolean)
      : cmo.commodityId
        ? [cmo.commodityId]
        : [];
...
export function mergeTicketFromApi(prev, row) {
  const mapped = ticketFromApi(row);
  if (!mapped) return prev;
  return {
    ...prev,
    ...mapped,
    ticketReference: mapped.ticketReference || prev?.ticketReference || "",
  };
}
...
export function ticketFromApi(row) {
  ...
  const truck =
    row.truck && typeof row.truck === "object"
      ...
      : null;
  const truckDisplay = typeof row.truck === "string" ? row.truck : truck?.name || truck?.driver || row.truckDisplay || "";
  ...
  return {
    ...
    weighbridgeId: row.weighbridge_id ?? row.weighbridgeId ?? row.weighbridge?.id ?? null,
    truck,
    truckDisplay,
    truckDriver: row.truck_driver ?? row.truckDriver ?? truck?.driver ?? "",
    customerId: row.customer_id ?? row.customerId ?? row.cmo?.customer_id ?? null,
    customerName: row.customer_name ?? row.customerName ?? row.cmo?.customer?.name ?? "",
    customerCode: row.customer_code ?? row.customerCode ?? row.cmo?.customer?.code ?? "",
    cmoReference: row.cmo_reference ?? row.cmoReference ?? row.cmo?.cmo_reference ?? "",
    ...
    commodityDescription: row.commodity_description ?? row.commodityDescription ?? "",
    ...
    ngr: row.ngr ?? "",
    season: row.season ?? row.cmo?.season ?? "",
    shrinkTakenMt: row.shrink_taken_mt ?? row.shrinkTakenMt ?? null,
    shrinkPercent: row.shrink_percent ?? row.shrinkPercent ?? null,
    ...
    truckArrivedAt: row.truck_arrived_at ?? row.truckArrivedAt ?? null,
    truckDepartedAt: row.truck_departed_at ?? row.truckDepartedAt ?? null,
    truckTurnaroundMinutes:
      row.truck_turnaround_minutes ?? row.truckTurnaroundMinutes ?? null,
    truckOnSiteMinutes: row.truck_on_site_minutes ?? row.truckOnSiteMinutes ?? null,
  };
}
...
export function ticketToApi(ticket) {
  ...
  return {
    ...
    weighbridge_id: ticket.weighbridgeId || null,
    ...
    booking_ref: ticket.bookingRef || null,
    ticket_ref: ticket.ticketRef || null,
...
export async function fetchTickets({ type, status, date, dateFrom, dateTo, perPage } = {}) {
...
function ticketMatchesTruck(row, truckId, truckName) {
  const rowTruckId = row.truckId || row.truck?.id;
  if (truckId && rowTruckId && String(rowTruckId) === String(truckId)) return true;
  const rowRego =
    typeof row.truck === "string"
      ? row.truck
      : row.truck?.name || row.truckDisplay || "";
  if (!truckName || !rowRego) return false;
  return String(rowRego).trim().toUpperCase() === String(truckName).trim().toUpperCase();
}

export async function fetchDayTicketsForTruck({ type, date, truckId, truckName, excludeId } = {}) {
  if (!date || (!truckId && !truckName)) return [];
  const list = await fetchTickets({ type, date, perPage: 500 });
  const candidates = list.filter((row) => {
    if (excludeId && String(row.id) === String(excludeId)) return false;
    return ticketMatchesTruck(row, truckId, truckName);
  });
  if (!candidates.length) return [];
  const fullTickets = await Promise.all(candidates.map((row) => fetchTicket(row.id)));
  return fullTickets.filter(Boolean);
}

export async function lookupTruckWeightDuplicateTickets(ticket, { ticketType = "in", excludeId } = {}) {
  const truckId = ticket?.truckId || ticket?.truck?.id;
  const date = String(ticket?.date ?? "").slice(0, 10);
  if (!truckId || !date) return [];

  const dayTickets = await fetchDayTicketsForTruck({
    type: ticketType,
    date,
    truckId,
    truckName: ticket?.truck?.name,
    excludeId,
  });

  return findDuplicateTickets(ticket, dayTickets, { ticketType, excludeId });
}
...
export async function overrideTicket(id, prevTicket = null) {
  const data = await ticketingRequest(`${TICKETS_ENDPOINT}/${id}/override${getTenantQuery()}`, {
    method: "POST",
    body: JSON.stringify({}),
  });
  return mergeTicketFromApi(prevTicket, data);
}
...
export async function saveTicket(ticket) {
  ...
  if (ticket.id) {
    ...
    return mergeTicketFromApi(ticket, data);
  }
...
export async function completeTicket(id, prevTicket = null) {
...
export function truckFromApi(row) {
  ...
  return {
    ...
    driver: row.driver ?? "",
    tare: row.tare ?? null,
    avgGross: row.avg_gross ?? row.avgGross ?? null,
...
function truckToApi(truck) {
  ...
  const avgGrossRaw = truck.avgGross ?? truck.avg_gross;
  let avg_gross = null;
  ...
  return {
    ...
    rego: String(truck.name ?? truck.rego ?? "").trim().toUpperCase() || null,
...
export async function fetchCmoFormData() {
  ...
  return {
    customers: unwrapList(customersResult).filter((c) => !(c.is_shrink ?? c.isShrink ?? c.is_write_off ?? c.isWriteOff)),
