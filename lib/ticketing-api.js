...
export function ticketFromApi(row) {
  ...
  const truck = row.truck
    ? {
        id: row.truck.id,
        name: row.truck.rego ?? row.truck.name ?? "",
        driver: row.truck.driver ?? row.driver_name ?? "",
        tare: row.truck.tare ?? null,
      }
    : null;
...
export async function fetchTicketFormData(direction = "incoming") {
  ...
  const data = await ticketingRequest(`${TICKETS_ENDPOINT}/form-data?${params.toString()}`);
  return {
    cmos: (data.cmos ?? []).map(cmoFromApi),
    customers: data.customers ?? [],
    internalAccounts: data.internalAccounts ?? [],
    commodityTypes: (data.commodityTypes ?? []).map((t) => ({
      id: t.id,
      name: t.name,
    })),
    ...
    stockLocations: (data.stockLocations ?? []).map((l) => ({
      id: l.id,
      name: l.name,
    ...
    users: data.users ?? [],
    completedTickets: (data.completedTickets ?? []).map(ticketFromApi),
  };
}
