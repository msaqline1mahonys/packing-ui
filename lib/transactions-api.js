...
export function isSystemSinkCustomer(c) {
  if (!c) return false;
  return Boolean(c.is_shrink ?? c.isShrink ?? c.is_write_off ?? c.isWriteOff);
}
...
export function transactionFromApi(row) {
  ...
  return {
    ...
    ticketReference: row.ticketReference ?? row.ticket_reference ?? row.ticket?.ticket_reference ?? "",
...
export function balanceFromApi(row) {
  ...
  return {
    ...
    commodityTypeId: row.commodityTypeId ?? row.commodity_type_id ?? "",
    commodityTypeName: row.commodityTypeName ?? row.commodity_type_name ?? "",
    locationId: row.locationId ?? row.location_id,
...
export async function fetchAccountBalances({
  ...
  excludeShrinkAccounts,
  excludeWriteOffAccounts,
} = {}) {
  const params = new URLSearchParams(getTenantQuery().replace("?", ""));
  if (accountId) params.set("account_id", accountId);
  if (commodityId) params.set("commodity_id", commodityId);
  if (commodityTypeId) params.set("commodity_type_id", commodityTypeId);
  if (locationId) params.set("location_id", locationId);
  if (excludeSystemAccounts) params.set("exclude_system_accounts", "1");
  if (excludeShrinkAccounts) params.set("exclude_shrink_accounts", "1");
  if (excludeWriteOffAccounts) params.set("exclude_write_off_accounts", "1");
  const qs = params.toString();
  const data = await transactionsRequest(`${TRANSACTIONS_ENDPOINT}/balances${qs ? `?${qs}` : ""}`);
  return unwrapList(data).map(balanceFromApi);
}

export async function fetchShrinkAccounts() {
  const params = new URLSearchParams(getTenantQuery().replace("?", ""));
  params.set("per_page", "500");
  const data = await transactionsRequest(`${CUSTOMERS_ENDPOINT}?${params.toString()}`);
  return unwrapList(data)
    .filter((c) => c.is_shrink ?? c.isShrink)
    .map((c) => ({ key: c.id, name: c.name, accountType: "customer" }));
}

export async function fetchWriteOffAccounts() {
  const params = new URLSearchParams(getTenantQuery().replace("?", ""));
  params.set("per_page", "500");
  const data = await transactionsRequest(`${CUSTOMERS_ENDPOINT}?${params.toString()}`);
  return unwrapList(data)
    .filter((c) => c.is_write_off ?? c.isWriteOff)
    .map((c) => ({ key: c.id, name: c.name, accountType: "customer" }));
}

export async function fetchLocationUtilization({ excludeSystemAccounts } = {}) {
  const params = new URLSearchParams(getTenantQuery().replace("?", ""));
  if (excludeSystemAccounts) params.set("exclude_system_accounts", "1");
  const qs = params.toString();
  const data = await transactionsRequest(
    `${TRANSACTIONS_ENDPOINT}/location-utilization${qs ? `?${qs}` : ""}`
  );
...
export function formatTransactionRow(t) {
  ...
  return {
    ...t,
    date,
    ticketDisplay: t.ticketReference || (t.ticketId ? String(t.ticketId).slice(0, 8) : "-"),
...
export function computeTransactionTotals(rows, { activeOnly = false } = {}) {
  ...
  // Count only the deduction side (negative) so the shrink credited to the
  // dedicated Shrinkage customer account is not double-counted.
  const shrinkage = source
    .filter((t) => t.transactionType === "shrinkage" && t.accountType === "customer" && t.quantity < 0)
    .reduce((s, t) => s + Math.abs(t.quantity), 0);
  const writeOffByRef = {};
  for (const t of source) {
    if (t.transactionType !== "write_off" || !t.reference) continue;
    const absQty = Math.abs(Number(t.quantity));
    if (!Number.isFinite(absQty)) continue;
    writeOffByRef[t.reference] = Math.max(writeOffByRef[t.reference] || 0, absQty);
  }
  const writeOffs = Object.values(writeOffByRef).reduce((s, v) => s + v, 0);
  const net = source.reduce((s, t) => s + t.quantity, 0);
  return { deposits, withdrawals, shrinkage, writeOffs, net };
}
