...
function appendFormPayload(form, payload) {
  Object.entries(payload).forEach(([key, value]) => {
    ...
    if (Array.isArray(value)) {
      value.forEach((item) => {
        if (item != null && item !== "") form.append(`${key}[]`, String(item));
      });
      return;
    }
...
function buildUserSaveBody(payload, { signatureFile, clearSignatureFile, method } = {}) {
  if (signatureFile instanceof File) {
    ...
    return { body: form, method: method === "PUT" ? "POST" : method || "POST" };
  }
  return {
    body: clearSignatureFile ? JSON.stringify({ ...payload, signature_file: null }) : JSON.stringify(payload),
    method: method || "POST",
  };
}
...
export async function createApiUser({
  ...
  const save = buildUserSaveBody(payload, { signatureFile, clearSignatureFile, method: "POST" });
...
export async function updateApiUser(id, data) {
  ...
  const save = buildUserSaveBody(payload, {
    signatureFile: data.signatureFile,
    clearSignatureFile: data.clearSignatureFile,
    method: "PUT",
  });
