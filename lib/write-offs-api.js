...
const WRITE_OFFS_ENDPOINT = `${API_BASE_URL}/write-offs`;
...
export async function createWriteOff({
  ...
  const response = await fetch(WRITE_OFFS_ENDPOINT, {
    method: "POST",
  ...
  if (!response.ok || result?.success === false) {
    throw new Error(extractApiError(result, "Failed to create write-off."));
  }
